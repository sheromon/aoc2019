...
def problem14a(reaction_spec=None):
    if reaction_spec is None:
        file_name = 'problem14.txt'
        with open(file_name) as file_obj:
            reactions = read_reactions(file_obj)
    else:
        reactions = read_reactions(reaction_spec)
...
def problem14b(reaction_spec=None):
    if reaction_spec is None:
        file_name = 'problem14.txt'
        with open(file_name) as file_obj:
            reactions = read_reactions(file_obj)
    ...
    factory = Factory(reactions)
    initial_ore = int(1e12)
    ore_used = []
    while not ore_used or factory.has_leftovers:
        num_ore = factory.calc_ore_needed('FUEL', 1)
        ore_used.append(num_ore)
        if sum(ore_used) >= initial_ore:
            break
    ore_to_get_no_leftovers = sum(ore_used)

    num_batches = int(initial_ore // ore_to_get_no_leftovers)
    num_fuel = num_batches * len(ore_used)
    remaining_ore = initial_ore - num_batches * ore_to_get_no_leftovers
    cum_ore_needed = np.cumsum(ore_used)
    additional_fuel = np.argmax(cum_ore_needed > remaining_ore)
    num_fuel += additional_fuel
    # print('ore_to_get_no_leftovers:', ore_to_get_no_leftovers)
    # print('num_batches:', num_batches)
    # print('remaining_ore:', remaining_ore)
    # print('num_fuel:', num_fuel)
    return num_fuel
...
class Factory():
    ...
    @property
    def has_leftovers(self):
        return any(self.leftovers.values())
...
def test_problem14b():
    ...
    num_fuel = problem14b(test_input)
    assert num_fuel == 82892753
